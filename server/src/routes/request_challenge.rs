...
pub async fn route(State(state): State<Arc<AppState>>) -> Result<Json<Challenge>, ApiError> {
    let challenge = generate_challenge(&state).await?;

    state
        .challenge_cache
        .insert(
            format!("challenge:{}", challenge.prefix),
            challenge.target.to_owned(),
        )
        .await;
...
async fn generate_challenge(state: &Arc<AppState>) -> Result<Challenge> {
    ...
    let base_target =
        b"000000FF00000000000000000000000000000000000000000000000000000000".to_owned();
